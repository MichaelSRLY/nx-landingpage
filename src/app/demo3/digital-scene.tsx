...
type Signal = { id: string; label: string; unit: string; stage: 1 | 2 | 3 | 4 };
  ...
  return (
    <>
      ...
      >
        ...
        <TopNav />
        <BOMRail signals={signals} progress={progress} />
    ...
    <header style={sx.nav}>
      <div className="nx-nav-inner" style={sx.navInner}>
        <div style={sx.brand}>
          ...
          <span className="nx-brand-sub" style={sx.brandSub}>Digital Twin · Live</span>
        </div>
...
function BOMRail({
  ...
  // Signals come online in stages as the camera rises through the ceiling
  const stageOf = (p: number) => (p < 0.25 ? 1 : p < 0.55 ? 2 : p < 0.8 ? 3 : 4);
  const cur = stageOf(progress);
  // Pulse value 0..1 — used as fake live-value bar width
  const pulse = (seed: number) =>
    0.55 + 0.35 * Math.sin(progress * Math.PI * 4 + seed * 1.3);
  return (
    <aside className="nx-bom-rail" style={sx.rail}>
      <div style={sx.railGlass}>
        <div style={sx.railHeader}>
          <span style={sx.railHeaderLabel}>Live telemetry</span>
        ...
        <ul style={sx.railList}>
          {signals.map((s, i) => {
            const online = s.stage <= cur;
            const activeNow = s.stage === cur;
            const value = online ? pulse(i) : 0;
            return (
              <li key={s.id} style={sx.railItem}>
                <span
                  style={{
                    ...sx.railDot,
                    background: online ? GOLD : "rgba(255,255,255,0.18)",
                ...
                <span style={sx.railItemBody}>
                  ...
                  <span style={sx.railBarWrap}>
                    <span
                      style={{
                        ...sx.railBarFill,
                        width: `${value * 100}%`,
                        opacity: online ? 1 : 0.15,
                      }}
                    />
                  </span>
                </span>
                <span
                  style={{
                    ...sx.railItemCheck,
                    color: online ? GOLD : "rgba(255,255,255,0.15)",
                  }}
                >
                  {online ? "●" : "○"}
                </span>
    ...
    <div style={sx.bottomMeta}>
      <div className="nx-bottom-meta-extras" style={sx.metaLeft}>
        ...
        <span style={sx.metaValue}>
          {progress < 0.33 ? "OFFLINE" : progress < 0.7 ? "SYNCING" : "OPERATIONAL"}
        </span>
      </div>
      ...
      <div className="nx-bottom-meta-extras" style={sx.metaRight}>
        ...
        <span style={sx.metaValue}>NX-DT · Digital Twin</span>
      </div>
    </div>
  );
}
...
const sx: Record<string, React.CSSProperties> = {
  ...
  railBarWrap: { display: "block", marginTop: 4, height: 2, width: "100%", background: "rgba(255,255,255,0.08)", overflow: "hidden" },
  railBarFill: { display: "block", height: "100%", background: GOLD, transition: "width 300ms ease, opacity 200ms", boxShadow: `0 0 8px ${GOLD_SOFT}` },
